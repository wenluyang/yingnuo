$('#cmd-slideshow-save').click(function () {
    ...
    if($id==0){
        var $url=common_ops.buildAdminUrl('/banner/store')
    }else {
        var $url=common_ops.buildAdminUrl('/banner/###/update').replace('###', $id)
    }
        ...
        function (response) {
            if (response.status) {
                layer.msg('保存成功', {time: 1000},function () {
                    window.location.href=common_ops.buildAdminUrl('/banner');
                });
            }
        },
        'json'
...
$('.cmd-slideshow-delete').click(function () {
    ...
    layer.confirm('确认删除？', {icon: 3, title: '提示'}, function (index) {
        $.post(
            common_ops.buildAdminUrl('/banner/###/delete').replace('###', $this.parent().data('id')),
            {_method: 'delete'},
            function (response) {
                if (response.status) {
...
$('.cmd-sortord').click(function () {
    ...
    layer.prompt({value: $this.data('sortord'), title: '请输入序号'}, function (sort){
        ...
        $.post(
            common_ops.buildAdminUrl('/banner/###/sort').replace('###', $this.parent().data('id')),
            {_method: 'POST', sort},
            function (response) {
                if (response.status) {
