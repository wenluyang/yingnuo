$('#cmd-cate-save').click(function () {
    ...
    if($id==0){
        var $url= common_ops.buildAdminUrl('/newscat/store');
    }else{
        var $url=common_ops.buildAdminUrl('/newscat/###/update').replace('###', $id)
    }
        ...
        function (response) {
            if (response.status) {
                layer.msg('保存成功', {time: 1000},function () {
                    window.location.href=common_ops.buildAdminUrl('/newscat');
                });
            }
        },
        'json'
...
$('#category-tags').on('click', '.cmd-category-tag-hide', function () {
    ...
    $.post(
        common_ops.buildAdminUrl('/newscat/###/hide').replace('###', $this.parent().data('id')),
        {_method: 'PATCH'},
        function (response) {
            if (response.status) {
...
$('#category-tags').on('click', '.cmd-category-tag-show', function () {
    ...
    $.post(
        common_ops.buildAdminUrl('/newscat/###/display').replace('###', $this.parent().data('id')),
        {_method: 'PATCH'},
        function (response) {
            if (response.status) {
...
$('#category-tags').on('click', '.cmd-category-tag-delete', function () {
    ...
    layer.confirm('确认删除？', {icon: 3, title: '提示'}, function (index) {
        $.post(
            common_ops.buildAdminUrl('/newscat/###/delete').replace('###', $this.parent().data('id')),
            {_method: 'delete'},
            function (response) {
                if (response.status) {
...
$('.cmd-set-sort').click(function () {
    ...
    layer.prompt({value: $this.data('sort'), title: '请输入正确的序号'}, function (sort) {
        ...
        $.post(
            common_ops.buildAdminUrl('/newscat/###/sort').replace('###', $this.data('id')),
