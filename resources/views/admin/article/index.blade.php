...
@section('content')
    <div class="row">
        <div class="col-md-3 col-xs-3">
            <a class="btn btn-success" href="{{route('admin.article.create')}}"><span class="glyphicon glyphicon-plus"></span> 新增</a>
        </div>
    ...
    <table class="table table-striped table-hover">
        ...
        @foreach($articles as $list)
            <tr>
                ...
                <td>{{$list->belongsToArticleCat->name}}</td>
                <td>{{$list->user_id==0?'管理员':'这里需要与user关联'}}</td>
                <td>{{$list->sort}}</td>
                ...
                <td data-id="{{$list->id}}">
                    ...
                    <a href="{{route('admin.article.show',['id'=>$list->id])}}" class="btn btn-xs btn-primary">详情</a>
...
@section('js')
    <script type="text/javascript" src="{{asset_url('js/admin/article_list.js')}}"></script>
@stop