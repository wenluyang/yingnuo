    ...
    <div class="container-fluid">
        <div class="collapse navbar-collapse">
            <div class="navbar-header active">
                <a class="navbar-brand" href="/">{{ config('app.name') }}后台管理</a>
            </div>
            <ul class="nav navbar-nav">
                @foreach ($nav as $moduleKey => $module)
                    <li class="nav-sub{{ $moduleKey == $navActive['module'] ? ' active' : '' }}">
...
@if (isset($nav[$navActive['module']]['functions']))
    <div class="sidenav">
        <ul class="menu">
            @foreach ($nav[$navActive['module']]['functions'] as $functionKey => $function)
                @if (isset($function['separator']))
                    <li class="divider"></li>
                @else
                    @if ($function['display'])
                        @if ($functionKey == $navActive['function'])
                ...
                @endif
            @endforeach
            @if (isset($nav[$navActive['module']]['functions'][$navActive['function']]) && !$nav[$navActive['module']]['functions'][$navActive['function']]['display'])
                <li class="divider"></li>
                @if (isset($nav[$navActive['module']]['functions'][$navActive['function']]['actions']))
                    <li class="active"><a href="javascript:window.history.back();">{{ $nav[$navActive['module']]['functions'][$navActive['function']]['name'] }}</a></li>
                    @foreach ($nav[$navActive['module']]['functions'][$navActive['function']]['actions'] as $actionKey => $action)
                        @if ($actionKey == $navActive['action'])
                            <li class="active sub"><span>{{ $nav[$navActive['module']]['functions'][$navActive['function']]['actions'][$navActive['action']]['name'] }}</span></li>
                        @endif
                    @endforeach
                @else
                    <li class="active"><span>{{ $nav[$navActive['module']]['functions'][$navActive['function']]['name'] }}</span></li>
                @endif
            @endif
        </ul>
    </div>
